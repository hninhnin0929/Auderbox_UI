...
@Component({
  ...
  templateUrl: './loyalty-report.page.html',
  styleUrls: ['./loyalty-report.page.scss'],
})

export class LoyaltyReportPage implements OnInit {
  ...
  search(currentIndex){
  const url =this.manager.appConfig.apiurl +'reports/loyaltyreport';
     ...
     this.http.post(url, this.obj, this.manager.getOptions()).subscribe(
      (data:any)  => {
        ...
        if(data.message == "SUCCESS"){
          ...
          let excelHeaderData = [
            "Date", "Shop Account", "Shop Name", "Brand Owner",
            "Invoice Amount", "Net Discount Amount", "Gift",
            "Discount"
          ];
          ...
          for(var exCount = 0; exCount < data1.length; exCount++){
            ...
            excelData.push(data1[exCount].shopacc);
            ...
            excelData.push(data1[exCount].gift);
