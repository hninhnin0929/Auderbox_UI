...
@Component({
  ...
  templateUrl: './packtype.page.html',
  styleUrls: ['./packtype.page.scss'],
})
export class PacktypePage implements OnInit {
  btn:boolean = false;
  list: any = [];
  obj: any = this.getObj();
  ...
  getAll() {
    ...
    this.spinner = true;
    let status = "";
    const url = this.manager.appConfig.apiurl + 'StockSetup/searchPackTypeList';
  ...
  save() {
    ...
    this.loading.create(
      ...
      el => {
        var status="";
        el.present();
        const url = this.manager.appConfig.apiurl + 'StockSetup/savepacktype';
        ...
        this.http.post(url, param, this.manager.getOptions()).subscribe(
          (data: any) => {
            ...
            if (data.message == "SUCCESS") {
              ...
              let excelTitle = "PackType Report";
              let excelHeaderData = [
              ...
              let worksheet = workbook.addWorksheet('PackType Data');
              ...
              workbook.xlsx.writeBuffer().then((data) => {
                ...
                FileSaver.saveAs(blob, "packType_export_" + new Date().getTime() + EXCEL_EXTENSION);
              });
            }
          }
        );
